...
FoundProcess:               '見つかった時
    ...
    Private Sub Button6_Click(sender As Object, e As EventArgs) Handles Button6.Click    '----Start Btn
        ...
        'getrow = 生徒TABL.NewRow

        Dim rownum As Integer = 1
        For Each getrow As DataRow In 生徒TABL.Rows
            Select Case rownum = 生徒TABL.Rows.Count
                Case True
                    MessageBox.Show("最終行　" + getrow("FirstName") + getrow("SecondName"))
                Case Else
                    MessageBox.Show(getrow("ID") + "行目　" + getrow("FirstName") + getrow("SecondName"))
                    rownum = rownum + 1
            End Select
        Next
        CmbTablList.Select()
    ...
    ' Public Shared Functionの練習
    Public Shared Function getName()
        Return "test"
    End Function
    Public Property rgbParameter As String()
    Public Shared Function rtnParameter(ByVal hexNum As String) As String()
        Dim P(2) As String
